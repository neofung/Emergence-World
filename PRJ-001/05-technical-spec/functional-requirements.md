...
**功能描述：**
通过 i18n 资源文件支持多语言。代理 display_name、personality、north_star_goal、地标 tagline/description/folklore/fun_fact 均从 JSON 资源文件加载。系统提示词根据语言设置切换模板。

**数据模型：**

| 资源文件 | 内容 |
|---------|------|
| `seed/i18n/en.json` | 英文翻译 |
| `seed/i18n/zh_cn.json` | 中文翻译 |

**业务规则：**
1. 通过 `LANGUAGE` 环境变量切换语言（默认 `zh_cn`）
2. 种子加载时根据语言配置合并翻译到数据库
3. 系统提示词支持 en/zh_cn 两种模板
4. zh_cn 模板末尾强制指令：「你必须用中文回复」
...
**功能描述：**
记录代理的每次行动（工具调用、发言）到 EventLog 表，前端活动流面板混合显示对话和行动。
...
| event_type | string | speech / tool_call / error |
| tool_name | string | 工具名 |
| content | text | 行动内容 |
| location | string | 当前位置 |

**业务规则：**
1. 每次工具调用和文字输出都记录到 EventLog
2. 前端活动流按时间倒序混合显示对话（💬）和行动（🗣️⚡）
3. 后台日志同时显示 display_name
...
**功能描述：**
Canvas 沙盘支持缩放/平移、标签防重叠、地标分类图案、点击查看详情、重叠目标循环选择。

**业务规则：**
1. 滚轮缩放（0.5x~10x），以鼠标位置为中心
2. 左键拖拽平移，双击复位
3. 右下角缩放控件（+/-按钮 + 滑块 + 百分比 + 1:1复位）
4. 标签防重叠：迭代排斥算法，最多 30 轮
5. 地标图案按类别区分：住宅方块、商业菱形、市政五边形、休闲圆形、娱乐六边形、地标星形
6. 点击地标显示详情面板（名称、标语、状态、描述、传说、趣闻）
7. 重叠区域点击循环选择目标
8. Canvas 自适应容器大小，Retina 清晰渲染

---

### FR-022：代理 display_name 本地化
...
**功能描述：**
代理拥有本地化 display_name，前端所有位置（Canvas 标签、侧边栏、活动流）均显示 display_name。对话中的英文名自动替换为带颜色的 display_name。

**业务规则：**
1. display_name 存储在 i18n 资源文件中
2. zh_cn 使用 emoji + 中文昵称（如 🔥调停哥、🔨建造者）
3. Agent List 中代理名使用对应颜色高亮
4. 对话文本中自动替换并着色
...
| 2026-06-18 | v1.3 | 新增 FR-019~FR-022：i18n、活动流、2D增强、display_name | Phase 6 功能补全 |
